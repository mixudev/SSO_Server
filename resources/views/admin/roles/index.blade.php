<x-app-layout>
                                    ...
                                    <tr class="group hover:bg-gray-50/70 dark:hover:bg-gray-700/30 transition-colors duration-100">
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <button type="button"
                                                        onclick="openModal('deleteRoleModal{{ $role->id }}', '{{ route('admin.roles.destroy', $role) }}')"
    ...
    {{-- Modals untuk delete role --}}
    @foreach($roles as $role)
        <x-confirm-modal
            id="deleteRoleModal{{ $role->id }}"
            type="danger"
            title="Hapus Role"
            :message="'Hapus role \'' . $role->name . '\'?<br>Role ini juga akan dihapus dari ' . $role->users_count . ' user yang terkait.'"
